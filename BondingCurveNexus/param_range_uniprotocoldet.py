...
#-----GRAPHS-----#
def show_graphs():
    fig, axs = plt.subplots(5, 2, figsize=(15,27)) # axs is a (6,2) nd-array
    # Subplot
...
if __name__ == "__main__":
    ...
    lambda_entries_range = [50, 50]
    lambda_exits_range = [100, 1000]
    det_size_range = [5, 5, 0.5, 0.5]
    ...
    for i in range(2):
        ...
        label_names.append(f'{lambda_entries_range[i]} entries, {lambda_exits_range[i]} exits, {det_size_range[i*2]} ETH - protocol')
        label_names.append(f'{lambda_entries_range[i]} entries, {lambda_exits_range[i]} exits, {det_size_range[i*2]} ETH - markets')
