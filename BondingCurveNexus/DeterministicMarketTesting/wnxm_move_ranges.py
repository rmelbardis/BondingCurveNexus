...
def show_graphs():
    ...
    fig.suptitle('''Deterministic Market Model - varying buy pressure.
...
if __name__ == "__main__":
    ...
    wnxm_liquidities = [5e-7*0.5, 5e-7, 5e-7*2, 5e-7*3]

    sims = []
    label_names = []
    days = []

    for wnxm_liquidity in wnxm_liquidities:
        model_params.wnxm_move_size = wnxm_liquidity
        sims.append(UniMarketsDet())
        label_names.append(f'{wnxm_liquidity} ETH price movement per 1 ETH of market buys/sells')
