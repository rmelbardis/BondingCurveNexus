...
def main():
    ...
    run_name = "10,000TarLiq_100LiqInOut_5%Ratchet_6EntriesDay_5ExitsDay"
    # eth_reserve = 43_835
        ...
        for e in events_today:
            ...
            if e == 'buy':
                if ramm.getSpotPriceA()/1e18 > wnxm.wnxm_price and wnxm.wnxm_supply > 0:
                    wnxm.market_buy(n_wnxm=wnxm.arb_sale_size_nxm, remove=False)
                else:
                    ramm.swap(0, value=int(wnxm.arb_sale_size_nxm * wnxm.wnxm_price * 1e18), sender=dev)
    ...
    fig.suptitle(f'''Deterministic Market Model, Solidity Contracts
                 Target liq of {liq_prediction[0]} ETH
                 Ratchet speed = 5% of BV/day. Withdrawal and long-term injection at 100 ETH/day
                 {lambda_exits} {wnxm.arb_sale_size_nxm} NXM exits per day. {lambda_entries} {wnxm.arb_buy_size_eth} ETH exits per day.
                 ''',
                 fontsize=16)
    # fig.tight_layout()
